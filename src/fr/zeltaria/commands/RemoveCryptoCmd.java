...
public class RemoveCryptoCmd implements TabExecutor {

    private final Main main;

    public RemoveCryptoCmd(Main main) {this.main = main;}

    @Override
    public boolean onCommand(CommandSender sender, Command command, String s, String[] strings) {
        if (sender instanceof Player) {
            Player p = (Player) sender;
            if(p.hasPermission("crypto.removecrypto")){
                if(strings.length == 3){
                    ...
                    for (String crypto : all) {
                        ...
                        if (strings[1].equals(abv[0])) {
                            ...
                            if((double) configuration.get(cle) < parseDouble(strings[2])){
                                p.sendMessage(main.prefix+ ChatColor.RED+"Le joueur n'as pas autant de "+abv[0]);
                            }
                            else{
                                ...
                                p.sendMessage(main.prefix+ ChatColor.GREEN+"Vous avez enlevé "+ChatColor.GOLD+strings[2]+" "+abv[0]+ChatColor.GREEN+" au joueur "+ChatColor.GOLD+strings[0]);
                    ...
                    p.sendMessage(main.prefix+ ChatColor.RED+"Veuillez entrer la commande comme ceci: /removecrypto <joueur> <crypto> <montant>");
                }
            }
        }
        return false;
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String s, String[] strings) {
        if(sender.hasPermission("crypto.removecrypto")){
            if(strings.length == 1){
