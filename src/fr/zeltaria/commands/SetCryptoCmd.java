...
public class SetCryptoCmd implements TabExecutor {

    private final Main main;

    public SetCryptoCmd(Main main){this.main = main;}

    @Override
    public boolean onCommand(CommandSender sender, Command command, String s, String[] strings) {
        if (sender instanceof Player) {
            Player p = (Player) sender;
            if(p.hasPermission("crypto.setcrypto")){
                if(strings.length == 3){
                    ...
                    for (String crypto : all) {
                        ...
                        if (strings[1].equals(abv[0])) {
                            ...
                            configuration.set(cle, parseDouble(strings[2]));
                            try {
                                configuration.save(file);
                            } catch (IOException e) {
                                throw new RuntimeException(e);
                            }
                            p.sendMessage(main.prefix+ ChatColor.GREEN+"Le solde de "+ChatColor.GOLD+abv[0]+ChatColor.GREEN+"du joueur "+ChatColor.GOLD+strings[0]+ChatColor.GREEN+" est maintenant de "+ChatColor.GOLD+configuration.get(cle)+abv[0]);
                        }
                    }
                    if (find == 0) {
                    ...
                    p.sendMessage(main.prefix+ ChatColor.RED+"Veuillez entrer la commande comme ceci: /setcrypto <joueur> <crypto> <montant>");
                }
            }
        }
        return false;
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String s, String[] strings) {
        if(sender.hasPermission("crypto.setcrypto")){
            if(strings.length == 1){
