...
public class BuyCryptoCmd implements TabExecutor {

    private final Main main;

    public BuyCryptoCmd(Main main){
        this.main = main;
    }
                    ...
                    if (strings[0].equals(abv[0])) {
                        ...
                        while (true){
                            try {
                                ...
                                if(Main.getEconomy().getBalance(p) < total){
                                    p.sendMessage(main.prefix+ChatColor.RED+"Tu n'as pas assez d'argent pour acheter "+strings[1]+" "+abv[0]);
                                    break;
                                }
                                Main.getEconomy().withdrawPlayer(p,total);
                                p.sendMessage(main.prefix+ChatColor.GREEN+"Vous venez d'acheter "+ ChatColor.GOLD+strings[1]+" "+abv[0]+ChatColor.GREEN+" pour "+ChatColor.GOLD+total+"€");
